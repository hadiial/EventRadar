// File: app/user-submission-history.tsx
...
export default function UserSubmissionHistoryScreen() {
  const router = useRouter();
  const [searchQuery, setSearchQuery] = useState('');
  const [historyEvents, setHistoryEvents] = useState<EventItem[]>([]);
  const [loading, setLoading] = useState(true);

  // Fetch events from Firebase and FILTER by the logged-in user
  useEffect(() => {
    ...
    const unsubDb = onValue(eventsRef, (snap) => {
      ...
      Object.entries(raw).forEach(([key, val]) => {
        // Check if this event was created by the currently logged-in user
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.titleContainer}>
          <Ionicons name="document-text" size={28} color="#2F4454" />
          <Text style={styles.pageTitle}>Riwayat Pengajuan</Text>
        </View>
        ...
        {loading ? (
          <View style={styles.centerState}>
            ...
            <Text style={styles.centerText}>Memuat riwayat pengajuan...</Text>
          </View>
        ) : filtered.length === 0 ? (
          <View style={styles.centerState}>
            ...
            <Text style={styles.centerText}>
              {historyEvents.length === 0
                ? 'Anda belum pernah mengajukan event.'
