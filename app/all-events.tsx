...
// Helpers
// ---------------------------------------------------------------------------
/** Warna dot untuk event mock berdasarkan string status */
function getMockStatusColor(status: string): string {
  switch (status) {
...
// ---------------------------------------------------------------------------
// 6 Mock events (sama dengan index.tsx)
// ---------------------------------------------------------------------------
...
export default function AllEventsScreen() {
  ...
  useEffect(() => {
    ...
    const unsubDb = onValue(eventsRef, (snap) => {
      if (!snap.exists()) { setDbEvents([]); return; }
  ...
  // Filter mock events berdasarkan search
  ...
  // ---------------------------------------------------------------------------
  const renderMockCard = (event: MockEvent) => (
    <TouchableOpacity
      key={event.id}
    ...
    >
      ...
      <View style={styles.gridTextContainer}>
        ...
        <View style={[styles.statusDot, { backgroundColor: getMockStatusColor(event.status) }]} />
      </View>
    </TouchableOpacity>
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        {/* -------- SECTION: MOCK EVENTS (recommended) -------- */}
        ...
        <View style={styles.gridContainer}>
          {filteredMock.length > 0
            ? filteredMock.map(renderMockCard)
