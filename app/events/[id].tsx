...
import {
  ...
  StatusBar, ScrollView, Platform, Image, ActivityIndicator, Alert, Linking,
} from 'react-native';
...
export default function EventDetailScreen() {
  ...
  useEffect(() => {
    ...
    const unsubDb = onValue(eventRef, (snapshot) => {
      setLoading(false);
      if (!snapshot.exists()) { setDbEvent(null); return; }
  ...
  const dotColor = isDbEvent && dbEvent
    ? getDateStatusColor(dbEvent.endDate)
    : '#10B981';

  const hasPosterUrl = dbEvent?.posterUrl && dbEvent.posterUrl.startsWith('http');
        ...
        <View style={styles.descriptionCard}>
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.titleRow}>
              <Text style={[styles.descriptionTitle, { flex: 1 }]} numberOfLines={2}>
                {dbEvent.title}
              </Text>
            ...
            <Text style={styles.metaValue}>{dbEvent.startDate} — {dbEvent.endDate}</Text>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.titleRow}>
            <Text style={[styles.descriptionTitle, { flex: 1 }]}>
              {id === 'EVT-001' ? 'IT FAIR XIV'
               : id === 'EVT-002' ? 'Lomba Futsal'
               : id === 'EVT-003' ? 'Workshop UI/UX'
               : id === 'EVT-004' ? 'Bazar Kampus'
               : id === 'EVT-005' ? 'Bedah Buku'
               : id === 'EVT-006' ? 'Pentas Seni'
               : `Event ${id}`}
            </Text>
            <View style={[styles.statusDot, {
              backgroundColor:
                id === 'EVT-001' || id === 'EVT-004' ? '#10B981' :
                id === 'EVT-002' || id === 'EVT-005' ? '#F59E0B' :
                '#EF4444'
            }]} />
          </View>
...
const styles = StyleSheet.create({
  ...
  avatarPlaceholder: {
    ...
    backgroundColor: '#D9D9D9', marginRight: 10,
  },
